...
library(GEOquery)
library(tidyverse)
dir.create("./Z.Other_datasets/GSE113764_Neck_bulkRNAseq/", recursive = T)

# load counts table from GEO
urld <- "https://www.ncbi.nlm.nih.gov/geo/download/?format=file&type=rnaseq_counts"
path <- paste(urld, "acc=GSE113764", "file=GSE113764_raw_counts_GRCh38.p13_NCBI.tsv.gz", sep="&");
tbl <- as.matrix(data.table::fread(path, header=T, colClasses="integer"), rownames="GeneID")
write.csv(tbl, "./Z.Other_datasets/GSE113764_Neck_bulkRNAseq/raw_counts.csv")

#load metadata
gse <- getGEO("GSE113764", destdir = getwd())
phenoData <- (pData(gse[[1]]))
rm(gse)
phenoData <- phenoData %>% select(title, geo_accession, `individual:ch1`, `tissue:ch1`,`18fdg-pet-ct:ch1`, platform_id)
write.csv(phenoData, "./Z.Other_datasets/GSE113764_Neck_bulkRNAseq/metadata.csv")
...
# sample selection
gsms <- "030303030303211212122121121212"
sml <- strsplit(gsms, split="")[[1]]

# group membership for samples
gs <- factor(sml)
groups <- make.names(c("Deep_UCP1neg","Deep_UCP1pos","SQ_BATpos","SQ_BATneg"))
levels(gs) <- groups
sample_info <- data.frame(Group = gs, row.names = colnames(tbl))

#Save raw counts
tT <-  merge(tbl, annot, by=0, sort = F)
...
plotDispEsts(ds, main="GSE113764 Dispersion Estimates")

# create histogram plot of p-values
hist(r$padj, breaks=seq(0, 1, length = 21), col = "grey", border = "white", 
     xlab = "", ylab = "", main = "GSE113764 Frequencies of padj-values")
...
par(mar=c(7,4,2,1))
boxplot(dat[,ord], boxwex=0.6, notch=T, main="GSE113764", ylab="lg(norm.counts)", outline=F, las=2, col=gs[ord])
...
ump <- umap(t(dat), n_neighbors = 13, random_state = 123)
