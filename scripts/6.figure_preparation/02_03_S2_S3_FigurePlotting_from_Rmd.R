# Converted from 2-3-S2-S3-FigurePlotting.Rmd
# RMarkdown text and chunk fences are commented out.
# Code chunks are preserved except for unnecessary/misplaced library() calls removed from setup.
...
save_pheatmap_pdf <- function(filename, width, height, expr) {
...
HEATMAP_CTS_RDS <- here(
    "..",
    "data/Figure_2_3_S2_S3/CamaraH_pseudobulk_count_matrix_for_ggplot.rds"
  )
...
ANGUEIRA_HEATvUCP1_CSV <- here(
  "..",
  "data/Figure_2_3_S2_S3/ssGSEA/Angueira_HEATvUCP1.csv"
)
...
venn
# Save PDF
ggsave(file.path(OUTPUT_DIR, "2A.2_venn_heat.pdf"), plot = venn, width = 30, height = 50, units = "mm", bg = "white", dpi = 600)
...
venn
# Save PDF
ggsave(file.path(OUTPUT_DIR, "2A.1_venn_heat.pdf"), plot = venn, width = 30, height = 50, units = "mm", bg = "white", dpi = 600)
# ```
#
#
...
sigscores_df |>
  distinct(Hallmark) |>
  filter(str_detect(Hallmark, "OXIDATIVE"))
sigscores_df |>
  filter(Hallmark %in% c(
    "HEAT",
...
pseudobulk_exp <- read_rds(HEATMAP_CTS_RDS)
# ```
#
#
# ```{r}
...
pseudobulk_exp_filtered <- pseudobulk_exp |>
  filter(
    cell_type_short %in% c("BAds", "WAds"),
    gene %in% thermo_genes,
    n_cells > 10
  ) |> # Filter to a minimum of cells per sample
...
plot_data_sig_score <- plot_data_sig_score |>
  mutate(
    ...
    region_extension = str_remove(region_label, " .*"),
    cell_type = case_when(
      str_detect(cell_type, "White") ~ "WAds",
...
make_one <- function(d) {
  {
    ...
    ggplot(df, aes(x = region_extension, y = sigScores, fill = cell_type_dataset)) +
      ...
      geom_hline(
        yintercept = wad_HEAT_percentile, color = "firebrick",
...
circles <- df %>%
  ...
  summarise(
    ...
    r = quantile(
      sqrt((umap_1 - median(umap_1, na.rm = TRUE))^2 +
...
# Close any accidentally open graphics devices before file-based plotting
...
save_pheatmap_pdf(
  filename = file.path(OUTPUT_DIR, "S3J_A38_heat_heatmap.pdf"),
  width = mm_to_in(25),
  height = mm_to_in(90),
  expr = {
    pheatmap::pheatmap(
      heatmap[custom.order, , drop = FALSE],
      cluster_cols = FALSE,
      cluster_rows = FALSE,
      cellwidth = 12,
      cellheight = 7.2,
...
a38.cells |>
  filter(Symbol %in% custom.order) |>
  select("Symbol", contains("D18"))
...
save_pheatmap_pdf(file.path(OUTPUT_DIR, "S3J_A41_heat_heatmap.pdf"),
  width = mm_to_in(25),
  height = mm_to_in(90),
  expr = {
...
save_pheatmap_pdf(file.path(OUTPUT_DIR, "S3J_cero_heat_heatmap.pdf"),
  width = mm_to_in(25),
  height = mm_to_in(90),
  expr = {
    pheatmap::pheatmap(heatmap[custom.order, , drop = FALSE],
...
ggsave(
  filename = file.path(OUTPUT_DIR, "S3K_heat_ucp1_corr_anguieira.pdf"),
  plot = gg_angueira_correlation,
  bg = "white",
  width = width,
  height = height,
  units = "mm",
...
ggsave(
  filename = file.path(OUTPUT_DIR, "S3N_heat_ucp1_corr_shamsi.pdf"),
  plot = gg_shamsi_correlation,
  bg = "white",
  width = width,
  height = height,
  units = "mm",
...
corr_used_adipocytes <- ucp1_v_sigscores |>
  filter(cell_type %in% c("Brown adipocytes", "Adipocytes"), Hallmark == "Mouse_HEAT") |>
  count(Ucp1 > 0)
corr_used_adipocytes
# ```
#
