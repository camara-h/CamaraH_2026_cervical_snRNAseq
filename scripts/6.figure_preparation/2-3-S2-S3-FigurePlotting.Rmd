...
# Outputs
OUTPUT_DIR <- here("output/6.figure_preparation/Figure_2_3_S2_S3")
...
mm_to_in(c(36, 32))
graph.data <- sigscores_df %>%
  filter(dataset == "CamaraH", Hallmark %in% c("UCP1 correlated", "Non_HEAT UCP1 correlated", "HEAT")) %>%
  left_join(meta_simplified, by = "barcode")
...
gg_ucp1_corr_featplot_annot <- ggplot(graph.data, aes(x = umap_1, y = umap_2, color = sigScores)) +
  geom_point(size = 0.1) +
  scale_color_viridis_c(option = "viridis", name = "Signature score (VISION)") +
...
## --- Plotting and save ---
```{r, fig.width=1.417323*4*3, fig.height=1.259843*4}
gg_ucp1_corr_featplot_annot

# Save
ggsave(file.path(OUTPUT_DIR, "REV_2C_ucp1_corr_vision_featplot.pdf"), plot = gg_ucp1_corr_featplot_annot, width = 36 * 3*3, height = 32 * 3, units = "mm", bg = "white",  dpi = 600)
ggsave(file.path(OUTPUT_DIR, "REV_2C_ucp1_corr_vision_featplot.png"), plot = gg_ucp1_corr_featplot_annot, width = 36 * 3*3, height = 32 * 3, units = "mm", bg = "white",  dpi = 600)
```

# --- Panel REVIEW. All UCP1 correlated Signature Violin Plot in CamaraH ---
## --- Read the Q99 HEAT percentile of White Adipocytes ---
...
theme_nature_metabolism() +
  ...
  facet_wrap(~Hallmark, ncol = 1, scales = "free_x") +
  scale_fill_manual(values = palette.use) +
  scale_color_manual(values = palette.use) +
  NoLegend()
```
...
## --- Plotting and save ---
```{r, fig.width=2.362205, fig.height=1.259843*3}
mm_to_in(c(60,32))
gg_heat_vln_allcells

scale <- 180
ggsave(file.path(OUTPUT_DIR, "REV_2D_heat_vln_allcells.pdf"), plot = gg_heat_vln_allcells, width = 60, height = 32*3, units = "mm", bg = "white",  dpi = 600)
ggsave(file.path(OUTPUT_DIR, "REV_2D_heat_vln_allcells.png"), plot = gg_heat_vln_allcells, width = 60, height = 32*3, units = "mm", bg = "white",  dpi = 600)
```
...
signatures_order <- c("UCP1 correlated", "HEAT", "Non HEAT UCP1 correlated" ,"FA METABOLISM", "OXPHOS", "BATLAS")
...
graph.data <- sigscores_df %>%
  ...
  mutate(Hallmark = str_remove(Hallmark, "H: "))
...
plot_data <- graph.data %>%
  ...
  left_join(meta_simplified, by = "barcode") %>%
  mutate(Hallmark_label = str_replace(Hallmark, pattern = "Non ", "non-"),
    Hallmark_label = str_wrap(as.character(Hallmark_label), 5)
  )
```
...
gg_vision_adipo_vln <- ggplot(
  ...
  vars(Hallmark),
  ncol = 6,
  labeller = labeller(Hallmark = label_wrap_gen(width = 5))
)
```
...
main_data <- plot_data %>%
  filter(Hallmark %in% signatures_main) %>%
  mutate(Hallmark = factor(Hallmark, levels = signatures_main)) |> 
  droplevels()

gg_vision_adipo_vln_main <- gg_vision_adipo_vln %+% main_data
gg_vision_adipo_vln_main

scale <- 180
ggsave(file.path(OUTPUT_DIR, "2D_vision_adipo_vln.pdf"), plot = gg_vision_adipo_vln_main, width = 80, height = 28.75, units = "mm", bg = "white",  dpi = 600)
```
## --- Review Plotting and save ---
```{r, fig.width=3.149606, fig.height= 1.13189}
mm_to_in(c(80, 28.75))
# 2. Figure-specific selections
signatures_review <- c(
...
main_data <- plot_data %>%
  filter(Hallmark %in% signatures_review) %>%
  mutate(Hallmark = factor(Hallmark, levels = signatures_review)) |> 
  droplevels()

gg_vision_adipo_vln_rev <- gg_vision_adipo_vln %+% main_data
gg_vision_adipo_vln_rev

scale <- 180
ggsave(file.path(OUTPUT_DIR, "REV_2D_vision_adipo_vln.pdf"), plot = gg_vision_adipo_vln_rev, width = 80, height = 28.75, units = "mm", bg = "white",  dpi = 600)
ggsave(file.path(OUTPUT_DIR, "REV_2D_vision_adipo_vln.png"), plot = gg_vision_adipo_vln_rev, width = 80, height = 28.75, units = "mm", bg = "white",  dpi = 600)
```
...
eff_plot_data <- eff_size_table %>%
  filter(feature == "gene_signature") %>%
  mutate(
    ...
    level = factor(
      level,
      levels = c("nuclei", "sample"),
      labels = c("Nuclei level", "Sample level")
    )
  )

gg_effect_size_2 <- ggplot(
  eff_plot_data,
  aes(
    x = cohen_d,
    y = fct_reorder(feature_label, cohen_d),
    shape = level
  )
) +
  geom_vline(
...
## --- Save plot ---
### --- Full version ---
```{r, fig.width=3.149606, fig.height=3.149606}
mm_to_in(c(80,80))
gg_effect_size_2

ggsave(file.path(OUTPUT_DIR, "REV_vision_effect_size.pdf"), plot = gg_effect_size_2, width = 80, height = 80, units = "mm", bg = "white",  dpi = 600)
ggsave(file.path(OUTPUT_DIR, "REV_vision_effect_size.png"), plot = gg_effect_size_2, width = 80, height = 80, units = "mm", bg = "white",  dpi = 600)
```

### --- Compact version ---

```{r, fig.width=2.362205, fig.height=1.574803}
mm_to_in(c(60,40))
signatures_review <- c(
  "UCP1 correlated",
  "HEAT",
...
main_data <- eff_plot_data %>%
  filter(feature_name %in% signatures_review)  |> 
  droplevels()

gg_effect_size_2_compact <- gg_effect_size_2 %+% main_data
gg_effect_size_2_compact

ggsave(file.path(OUTPUT_DIR, "REV_vision_effect_size_compact.pdf"), plot = gg_effect_size_2_compact, width = 60, height = 40, units = "mm", bg = "white",  dpi = 600)
ggsave(file.path(OUTPUT_DIR, "REV_vision_effect_size_compact.png"), plot = gg_effect_size_2_compact, width = 60, height = 40, units = "mm", bg = "white",  dpi = 600)
...
```{r}
sigscores_df |> distinct(Hallmark, dataset)
```
...
# --- Panel 3A-H. ssGSEA vs genes in bulkRNAseq ---
...
## --- Read ssGSEA and clean datasets ---
```{r}
# Remove Sondergaard
dataset_names_use <- grep(
  "Sondergaard",
  dataset_names,
  ignore.case = TRUE,
  invert = TRUE,
...
ssgsea_data <- setNames(
  lapply(dataset_names_use, function(dataset) {

    dat <- read.csv(str_subset(SSGSEA_FILES, dataset)) %>%
      rename(group = X.) %>%
      mutate(
    ...
    if (tolower(dataset) == "ojha") {
      dat <- dat %>%
        mutate(
          group = factor(
            group,
            levels = c("Neonate", "Infant", "Child")
          )
        )
    }
...
plot_signature_gene <- function(
  ...
  ggplot(
    ...
    labs(
      x = paste0(signature_label, " Score (ssGSEA)"),
      y = bquote(log[10](.(gene) + 1)),
      title = dataset_keys[dataset, ]
    )
}

# -----------------------------
#  Set up function to plot several datasets at once 
# -----------------------------


make_bulk_corrplots <- function(
  data_list,
  signatures = c(
    "HEAT"                     = "HEAT",
    "Non_HEAT.UCP1.correlated" = "non-HEAT",
    "UCP1.correlated"          = "UCP1-correlated"
  ),
  genes = c("UCP1", "CKMT2"),
  datasets = c("Gavrila", "Din", "Ohja", "Castella")
) {

  # Create all requested signature × gene combinations
  combinations <- tidyr::crossing(
    signature = names(signatures),
    gene = genes
  ) %>%
    mutate(
      signature_label = signatures[signature],
      plot_name = paste(signature_label, gene, sep = "_")
    )
  ...
  plots <- purrr::pmap(
    combinations,
    function(signature, gene, signature_label, plot_name) {

      dataset_plots <- purrr::map(
        datasets,
        function(dataset) {

          dat <- data_list[[dataset]]
          ...
          ggplot(
            ...
            theme(legend.position = "top") +
            scale_color_manual(
              values = color_palette_gsea_bulk
            ) +
            labs(
              x = str_wrap(paste0(signature_label, " Score (ssGSEA)"), 20),
              y = bquote(log[10](.(gene) + 1)),
              title = dataset_keys[dataset, ]
            )
        }
      ...
      # One row = one signature × gene combination
  ...
  names(plots) <- combinations$plot_name


  # --------------------------------------------------
  # Wrap all signature × gene combinations
  # --------------------------------------------------

  wrapped <- patchwork::wrap_plots(
    plots,
    ncol = 1
  )
...
## --- Plotting ---
```{r, fig.width=4.724409, fig.height=3.937008}
mm_to_in(c(120,100))
...
ucp1_corrplots <- make_bulk_corrplots(
  data_list = ssgsea_data,
  genes = c("UCP1")
)

ucp1_corrplots$wrapped

ggsave(here("output/99.figure_on_demand/0.review/bulk_corr_ucp1.png"), ucp1_corrplots$wrapped, width = 120, height = 100, units = "mm", dpi = 600, create.dir = TRUE)

# -----------------------------
#  CKMT2 
# -----------------------------
ckmt2_corrplots <- make_bulk_corrplots(
  data_list = ssgsea_data, 
  genes = c("CKMT2")
)

ckmt2_corrplots$wrapped

ggsave(here("output/99.figure_on_demand/0.review/bulk_corr_ckmt2.png"), ckmt2_corrplots$wrapped, width = 120, height = 100, units = "mm", dpi = 600, create.dir = TRUE)
```
# --- Panel 3A-H. ssGSEA vs UCP1 in bulkRNAseq ---
```{r}
gg_heat_bulk_corrplot <- make_bulk_corrplots(
  data_list = ssgsea_data, 
  signatures = c("HEAT" = "HEAT"),
  genes = c("UCP1")
)
```
...
```{r, fig.width=7.086614, fig.height= 1.771654, warning=FALSE}
# Wrap and save
gg_heat_bulk_corrplot$wrapped
scale <- 180
ggsave(file.path(OUTPUT_DIR, "3A-D_heat_ucp1_bulk_corrplot.pdf"), plot = gg_heat_bulk_corrplot$wrapped, width = 180, height = 225 / 5, units = "mm", bg = "white",  dpi = 600)
...
gg_heat_bulk_corrplot$wrapped
scale <- 180
ggsave(file.path(OUTPUT_DIR, "3E-H_heat_ckmt2_bulk_corrplot.pdf"), plot = gg_heat_bulk_corrplot$wrapped, width = 180, height = 225 / 5, units = "mm", bg = "white",  dpi = 600)
